...
{
    [BoxGroup("Trigger"), LabelText("트리거")]
    public BattleSpeechTrigger Trigger = BattleSpeechTrigger.BattleFlavor;

    [BoxGroup("Trigger"), LabelText("확률")]
    [Range(0f, 1f)] public float Chance = 0.35f;

    [BoxGroup("Condition"), LabelText("최소 전투 턴")]
    [MinValue(0)] public int MinBattleTurn = 0;

    [BoxGroup("Condition"), LabelText("HP 비율 이하")]
    [Range(0f, 1f)] public float HpRatioBelow = 1f;

    [BoxGroup("Condition"), LabelText("스킬 ID 필터")]
    [Tooltip("비워두면 모든 스킬에 반응합니다. SkillUse 트리거에서만 사용합니다.")]
    public string SkillIdFilter = "";

    [BoxGroup("Condition"), LabelText("한 번만 출력")]
    public bool TriggerOnce = false;

    [BoxGroup("Text"), TextArea(2, 4)]
    [Tooltip("사용 가능 토큰: {actor}, {target}, {skill}, {turn}, {hp}, {maxHp}")]
    public string Text = "...";
...
public class BattleSpeechBubble : MonoBehaviour
{
    ...
    [BoxGroup("Sizing"), SerializeField] private Vector2 _padding = new Vector2(48f, 30f);
    [BoxGroup("Sizing"), SerializeField] private Vector2 _minSize = new Vector2(120f, 56f);
    [BoxGroup("Sizing"), SerializeField] private Vector2 _maxSize = new Vector2(360f, 180f);
    ...
    public bool TryShow(BattleSpeechTrigger trigger, CharacterBase actor, SkillData skill = null, CharacterBase target = null, int battleTurn = 0, float holdOverride = -1f)
    {
        BattleSpeechRule rule = PickRule(trigger, actor, skill, battleTurn);
        if (rule == null) return false;

        if (rule.TriggerOnce) rule.HasTriggered = true;
        Show(Format(rule.Text, actor, skill, target, battleTurn), holdOverride > 0f ? holdOverride : _defaultHoldDuration);
        return true;
    }
    ...
    private BattleSpeechRule PickRule(BattleSpeechTrigger trigger, CharacterBase actor, SkillData skill, int battleTurn)
    {
        ...
        {
            BattleSpeechRule rule = _rules[i];
    ...
    private void ResizeToText(string text)
    {
        ...
        Vector2 preferred = _speechText.GetPreferredValues(text, _maxSize.x - _padding.x, _maxSize.y - _padding.y);
        Vector2 size = new Vector2(
            Mathf.Clamp(preferred.x + _padding.x, _minSize.x, _maxSize.x),
            Mathf.Clamp(preferred.y + _padding.y, _minSize.y, _maxSize.y));
