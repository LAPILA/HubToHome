...
Battle UI tween lifetime stays local to each view. `PartySlotUI` (standalone serializable class, unchanged field names) owns portrait, HP/AP, and punch handles; `BattleMenuUI` owns button color/scale, slide, and delayed input resume handles. Hide/rebind/disable/destroy kill only owned handles without completing gameplay callbacks. Captured Image/TMP references use Unity null guards, including component-only destruction; SetLink is a GameObject safety net, not the only cleanup. `BattleUIController` releases slots and its own panel/flash/shake handles and binds/unbinds the actual BattleManager event source. Do not use global KillAll or change DOTween Safe Mode to hide lifetime bugs.
