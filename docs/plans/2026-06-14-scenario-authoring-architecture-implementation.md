...
Current implementation note: `AssetDatabaseScenarioDialogueReferenceResolver` is the editor-side resolver/provider for this seam. It resolves `dialogueData` by `DialogueData` asset name, full `Assets/...` path, or path without `.asset`; scoped imports can pass search folders; duplicate name matches fail rather than choosing an arbitrary asset. `ScenarioDialogueReferenceData.DialogueDataId` preserves the source value, and `ScenarioSourceExporter` can export `BattleScenarioData` back to `ScenarioSourceDocument`. The remaining follow-up is writing that document back to YAML text through the chosen parser/writer and surfacing it in the editor.
...
2026-06-15 QTE module migration: The existing `turn_qte` path is now started through the Game Module Runner during battle setup. `BattleTurnQteGameModuleRuntime` accepts an `IBattleTurnQteModuleController`, and `BattleManager` injects a controller that owns lifecycle, turn calculation entry, turn advancement, player/enemy turn begin entry, enemy action entry, player action input, target confirmation, action completion, inactive-module guards, and pending QTE cleanup. The actual legacy animation, skill, and enemy attack/QTE resolution routines still live in `BattleManager`, but their module-facing entry points now go through the `turn_qte` module seam. Future QTE work should deepen this controller rather than adding setup/input/action-completion branches directly to `BattleManager`.
