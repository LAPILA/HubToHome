...
- Battle audio and screen presentation setup also flows through `BattleScenarioActionContextFactory`: `AudioManagerActionRunner` handles `bgm.crossfade` via `AudioManager`, resolving clip IDs through `BattleScenarioData.AudioClips` first and `ResourcesAudioClipResolver` fallback second, while `ScreenTransitionRunner` handles `screen.fade` through a runtime-created overlay.
