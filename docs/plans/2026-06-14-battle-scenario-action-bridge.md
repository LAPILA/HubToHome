...
- Battle audio and screen presentation setup also flows through `BattleScenarioActionContextFactory`: `AudioManagerActionRunner` handles `bgm.crossfade` via `AudioManager` and `ResourcesAudioClipResolver`, while `ScreenTransitionRunner` handles `screen.fade` through a runtime-created overlay.
