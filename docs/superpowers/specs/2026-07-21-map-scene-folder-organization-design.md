...
- 기획자와 맵 제작자가 `Assets/_Game/Content/Maps` 한 곳에서 모든 Unity 씬과 맵을 찾는다.
- 지역 씬, 해당 지역 전용 Prefab, 재질, 제작 메모를 하나의 지역 패키지로 묶는다.
- 타이틀, 인트로, 전투 씬도 맵과 같은 루트에서 명확한 용도별 폴더로 관리한다.
...
- `Maps/Frontend`: 타이틀과 인트로처럼 게임 시작 흐름에 쓰는 씬.
- `Maps/Battle`: 전용 전투 공간으로 사용하는 씬.
