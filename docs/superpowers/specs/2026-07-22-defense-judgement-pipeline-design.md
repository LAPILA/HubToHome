...
아래 본문은 기존 3입력 파이프라인의 설계 기록이다. 현재 기본 전투는 **Z 지속 가드·저스트 가드 / X 회피 / C 연계 반격**이다. 시간제 Z 방어는 호환 모드로 남겼다. 기존 판정·실행·피해 소비자 경계와 직렬화 번호·공격 스킬 Z/X/C 입력은 유지한다. 현행 동작·제작 순서·검증 범위는 [Z/X/C 구현 인계](../../../AIAssets/yjlim/Patchnote/2026-09-13-zxc-defense.md)를 따른다.
