...
아래 본문은 기존 3입력 파이프라인의 설계 기록이다. 기본 전투는 이제 **Z 시간제 방어 / 퍼펙트 패링**을 사용한다. 기존 판정·실행·피해 소비자 경계는 유지하며, 일반 방어의 부분 피해 결과를 추가했다. 구체적인 동작과 검증 범위는 [구현 기록](../plans/2026-09-07-timed-guard.md)을 따른다. 기존 요구 입력 데이터와 공격 스킬 Z/X/C는 보존한다.
