...
- [x] 0%, 50%, 100% 색 혼합 실패 테스트를 작성한다.
- [x] 비정상 강도값이 안전한 기본값으로 정규화되는 테스트를 작성한다.
- [x] `VisualAccessibilityPolicy.ScaleFlashColor`를 구현한다.
- [x] 정책 테스트를 통과시킨다.
...
- [x] 사용자 `TestMap.unity` 해시를 확인한다.
- [x] Jira에 결과를 기록하고 이번 파일만 로컬 커밋한다.
