...
- Scenario Source 대화 매핑은 `ScenarioSourceDialogueDocument` (`DialogueId`, `DialogueDataId`)로 표현하고, `IScenarioDialogueReferenceResolver`를 통해 `BattleScenarioData.Dialogues`로 import한다. 현재 에디터 기본 resolver는 `AssetDatabaseScenarioDialogueReferenceResolver`다. 이 resolver는 에셋 이름 또는 `Assets/...` 경로를 받으며, 선택 search folder 범위를 지키고, 같은 이름의 `DialogueData`가 여러 개면 추측하지 않고 실패해야 한다. 누락된 대화 참조는 null runtime mapping이 아니라 `scenario.dialogue.unresolved` validation error로 남겨야 한다.
