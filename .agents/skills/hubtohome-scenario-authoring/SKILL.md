...
- Battle camera presentation is owned by `ICameraPresentationService` / `CameraController` over Cinemachine. Author focus, reset, and impulse through `battle.camera.focus`, `battle.camera.reset`, and `battle.camera.shake`; use `static`, `dynamic`, or `gameplay_safe` shot styles instead of moving a CinemachineCamera Transform directly. `BattleManager` registers `PositionManager.CenterTransform` as the default target. `TimelineCutsceneRunner` acquires exclusive camera control for its playback lifetime and must release it on completion, cancellation, or destruction before optional camera reset.
...
- `ScenarioAuthoringWindow`는 현재 Korean UI Toolkit Sequence Maker surface다. `HubToHome/시나리오/시퀀스 메이커`에서 열고, `BattleScenarioData` 선택, optional `ActionCatalogAsset` 선택, 개요/규칙/시퀀스 요약, source stale 상태, catalog validation 메시지, YAML 미리보기, source path 또는 선택 path export, parser-backed source YAML 검증, top-level/child action reorder/insert/duplicate/disable/delete 조작, catalog 기반 action picker, row별 validation badge, source YAML edit-back 저장과 metadata sync, `런타임 에셋 반영` reimport를 제공한다. Reimport는 실패 시 runtime asset을 변경하지 않는 validation-first flow여야 한다.
