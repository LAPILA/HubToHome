...
- `BattleAimShooterGameModuleRuntime` is the first registered non-QTE module shell. It currently applies module presentation through `IBattleGameModulePresentationController`, disables legacy Turn QTE input, hides QTE menu/targeting/defense surfaces via `BattleUIController`, and sets a temporary `AIM SHOOTER` turn label. It does not yet implement mouse aiming, projectile spawning, damage, outcome reporting, or module-specific UI. Add those behind the `aim_shooter` runtime and `GameModuleRuntimeContext` seams rather than through `BattleManager` branches.
