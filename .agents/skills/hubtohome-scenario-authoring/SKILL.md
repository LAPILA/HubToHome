...
- Scenario Source dialogue mappings are represented by `ScenarioSourceDialogueDocument` (`DialogueId`, `DialogueDataId`) and imported through `IScenarioDialogueReferenceResolver` into `BattleScenarioData.Dialogues`. Missing dialogue references must produce `scenario.dialogue.unresolved`, not a null runtime mapping.
