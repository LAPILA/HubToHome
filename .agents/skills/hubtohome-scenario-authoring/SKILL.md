...
- `ScenarioAuthoringWindow`는 현재 Korean UI Toolkit editor surface다. `HubToHome/시나리오/시나리오 저작 창`에서 열고, `BattleScenarioData` 선택, optional `ActionCatalogAsset` 선택, 개요/규칙/시퀀스 요약, source stale 상태, catalog validation 메시지, YAML 미리보기, source path 또는 선택 path export, parser-backed source YAML 검증, top-level/child action reorder/insert/duplicate/disable/delete 조작, catalog 기반 action picker, row별 validation badge, source YAML edit-back 저장과 metadata sync를 제공한다. 아직 generated runtime asset reimport/replace는 구현하지 않았다.
