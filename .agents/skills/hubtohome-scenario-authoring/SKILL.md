...
- `ScenarioSourceExporter`는 `BattleScenarioData`를 `ScenarioSourceDocument`로 되돌리는 editor-side export seam이다. YAML writer가 붙기 전까지는 이 Document export를 round-trip 검증의 기준으로 삼는다.
