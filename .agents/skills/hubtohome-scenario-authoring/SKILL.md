...
- `BattleTurnQteGameModuleRuntime` is a compatibility module, not a full extracted combat module. It normalizes battle UI on enter/start and stops active QTE plus hides skill QTE UI on exit. It does not own turn calculation, player action selection, enemy action, or Battle Session State yet.
