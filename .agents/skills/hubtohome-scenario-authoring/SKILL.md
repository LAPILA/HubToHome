...
- `BattleSessionState` is the first explicit battle-scoped state object for facts that must survive Game Module switches. It currently records scenario identity, Primary Mode, opening module, and current module. `IGameModuleStateStore` is the narrow state seam used by `GameModuleActionRunner` to update the current module without making the runner own all battle state.
