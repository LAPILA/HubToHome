...
- The default battle bridge currently registers runtime-backed starter adapters: `flow.wait` and `dialogue.wait`. `dialogue.wait` IDs are resolved through `BattleScenarioData.Dialogues`, `ScenarioDialogueRegistry`, and `BattleScenarioActionContextFactory`; importer/editor work must keep Scenario Source dialogue mappings synchronized into that runtime list.
