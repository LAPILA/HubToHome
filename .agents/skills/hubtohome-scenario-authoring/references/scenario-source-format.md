...
- `ScenarioSourceYamlWriter` currently covers header fields, participants, `dialogues`, `audioClips`, `rules`, `sequences`, `flow.parallel`, and action `ParametersJson` object fields. Invalid action parameter JSON must produce `scenario.yaml.action.parameters.invalid`.
