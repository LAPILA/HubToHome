...
- `ScenarioAuthoringWindow` is the first Korean UI Toolkit surface. It opens from `HubToHome/시나리오/시나리오 저작 창`, reads a selected `BattleScenarioData`, accepts an optional `ActionCatalogAsset`, shows overview/rules/sequences/source stale state/catalog validation messages, previews YAML, and exports through `ScenarioSourceYamlExportCommand`.
- The remaining editor work is reorder/insert/duplicate/delete/light edit, richer per-row validation badges, and parser-backed reimport.
