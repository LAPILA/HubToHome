...
- A blocking Trigger or legacy Rule must expose `위험 작업 > 규칙 삭제` in its selected editor view. Both formats use `BattleScenarioEditCommandStack`, remain undoable, delete only the Rule, and return selection to the referenced Sequence.
