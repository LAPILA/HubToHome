...
- 콘텐츠 메이커의 대화 이름 변경은 `ContentMakerDialogueRenameGuard`로 시나리오 Runtime/Source의 대화 이름·경로 사용을 먼저 확인한다. 사용 중이면 변경을 차단하며 원문을 자동 수정하지 않는다. 대사 CSV는 한 `DialogueData`의 명시적 전체 교체이며 Scenario Source의 대체 원본이나 자동 동기화 경로가 아니다.
...
- A blocking Trigger or legacy Rule must expose `위험 작업 > 규칙 삭제` in its selected editor view. Both formats use `BattleScenarioEditCommandStack`, remain undoable, delete only the Rule, return selection to the referenced Sequence, and use a two-button `규칙 삭제 / 취소` confirmation without a placeholder third button.
- Rebuild `SequenceUsageIndex` after document edits that can change Rule targets or `sequence.call`; a removed reference must not remain as a stale deletion blocker until project refresh.
