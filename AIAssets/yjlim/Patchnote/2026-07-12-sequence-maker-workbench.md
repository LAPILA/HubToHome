...
- 대상 이동 경고를 `YAML 저장 후 이동 / 취소 / 저장하지 않고 이동`으로 명확화
...
- 실패한 저장은 미저장 표시와 복구 기록 유지
- 성공한 저장만 현재 시퀀스 복구 기록 정리
- 다른 시퀀스용 복구 기록 적용 차단
- 손상된 복구 metadata의 외부 파일 경로 접근 차단
- 고정 random seed 100개 편집과 전체 Undo/Redo 결과 검증
- 1,000 Block 구조 생성과 빠른 lookup 검증
- 안전 미리보기 실패, 성공, 중지, 재시작 상태 검증
